...
/**
 * REST controller for managing {@link org.beer30.mrpickles.domain.Post}.
 */
@RestController
...
public class PostResource {
    ...
    public Mono<ResponseEntity<Post>> createPost(@Valid @RequestBody Post post) throws URISyntaxException {
        ...
        return postRepository
            .save(post)
            .flatMap(postSearchRepository::save)
            .map(result -> {
                try {
    ...
    public Mono<ResponseEntity<Void>> deletePost(@PathVariable("id") Long id) {
        ...
        return postRepository
            .deleteById(id)
            .then(postSearchRepository.deleteById(id))
            .then(
                Mono.just(
    ...
    public Mono<ResponseEntity<Flux<Post>>> searchPosts(
        ...
        return postSearchRepository
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(postSearchRepository.search(query, pageable)));
    }
}
