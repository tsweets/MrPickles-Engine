...
/**
 * REST controller for managing {@link org.beer30.mrpickles.domain.Tag}.
 */
@RestController
...
public class TagResource {
    ...
    public Mono<ResponseEntity<Tag>> createTag(@Valid @RequestBody Tag tag) throws URISyntaxException {
        ...
        return tagRepository
            .save(tag)
            .flatMap(tagSearchRepository::save)
            .map(result -> {
                try {
    ...
    public Mono<ResponseEntity<Void>> deleteTag(@PathVariable("id") Long id) {
        ...
        return tagRepository
            .deleteById(id)
            .then(tagSearchRepository.deleteById(id))
            .then(
                Mono.just(
    ...
    public Mono<ResponseEntity<Flux<Tag>>> searchTags(
        ...
        return tagSearchRepository
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(tagSearchRepository.search(query, pageable)));
    }
}
